...
//        Assert.notNull(instance, "Instance must not be null");
	...
	/**
	 * 原生反射效率比较低，可以改为参数复制或者更高效率的其他反射框架
	 * 复制时忽略List类型和serialVersionUID
	 * @param source
	 * @param target
	 ...
	 */
    @SuppressWarnings("unchecked")
	public static void copyWithoutListAndSid(Object source, Object target)
			throws SecurityException,
			IllegalAccessException, IllegalArgumentException,
			InvocationTargetException {
		...
		for (Field sourceField : sourceFields) {
			...
			try {
				...
				for (Field targetField : targetFields) {
					String targetName = targetField.getName();// 目标对象的属性名
					Class targetType = targetField.getType();// 目标对象的属性名
					//属性名不同时找下一个
					if (!targetName.equals(name)) {
						continue;
					}
